 ...
 * 
 * // 原型链：对象的属性查找机制
// 每个实例都有一个属性 __proto__ 属性，它指向当前实例所属类的 prototype 对象。
    当我们访对象的一个的属性时，如果有，就使用私有属性，如果没有就通过实例 __proto__ 找到实例所属类的 prototype (原型)上查找，
    如果找到就使用 prototype 上的属性，如果还没找到，就通过 prototype 的 __proto__ 继续向上查找，一直找到 Object 的 prototype 就停止查找。
    如果还没找到就返回 undefined。
 */
