...
String.prototype.myReplace = function myReplace(reg, callback) {
  ...
  while (res) {
    ...
    // =>_this.substring(0, i) + returnV => 从0开始截取到第一次i(i=11)出现的位置(my name is) 然后加上returnV(周啸天哈哈)
    // _this.substring(v.length + i); 继续截取v.length + i 后面的字符串( v={0}, v.length = 3, i=11),又因为(my name is 周啸天哈哈)这个字符串的字符加起来超过了索引14的值，所以是从索引为14开始一直截取到末尾=>(哈哈 + i am {1} years old!)
    // 第一次循环匹配并替换后的结果为 => ('my name is 周啸天哈哈，i am {1} years old!)
    res = reg.exec(this);
  }
  return _this;
};
